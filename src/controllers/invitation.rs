...
pub async fn create_invitation(
    ...
    .bind(invitation.photographer_id)
    .fetch_optional(&state.pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    if photographer.is_none() {
        return Err((StatusCode::BAD_REQUEST, "Photographer does not exist".to_string()));
    }

    // Create the invitation
    sqlx::query(
        r#"
    INSERT INTO invitations (client_id, album_id, photographer_id)
    VALUES ($1, $2, $3)
    "#,
    )
    .bind(invitation.client_id)
